...
async def generate_exam_from_files(
    ...
    try:
        ...
        csv_dir = Path("uploads/csvs")
        csv_dir.mkdir(parents=True, exist_ok=True)
        csv_file_path = csv_dir / f"upload_{upload.id}.csv"
        
        with open(csv_file_path, "w", newline="", encoding="utf-8") as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(["question", "type", "options", "answer", "concepts"])
            ...
            for q in created_questions:
                ...
                if q.options and isinstance(q.options, dict):
                    opt_list = q.options.get("list", [])
                    if opt_list:
                        options_str = "|".join(str(opt) for opt in opt_list)
                
                # Format answer
                answer_str = ""
                if q.answer and isinstance(q.answer, dict):
                    answer_value = q.answer.get("value", "")
                    if isinstance(answer_value, list):
                        answer_str = "|".join(str(a) for a in answer_value)
                    else:
                ...
                if q.concept_ids:
                    for cid in q.concept_ids:
                        if cid in {concept_map[name].id: name for name in concept_map}:
                ...
                writer.writerow([question, qtype, options_str, answer_str, concepts_str])
        
        # Update database with file path
        upload.csv_file_path = str(csv_file_path)
        db.commit()
